...
impl SeamModel {
    ...
    /// `depth` (untimed), or mark a synthetic cached prefix when `synthetic_depth` is set, and time
    /// ONE metric: `pg` = a whole (P prefill + G decode) turn,
    ...
    pub fn bench_vulkan(
        ...
        let measured_depth = synthetic_depth.unwrap_or(depth);
